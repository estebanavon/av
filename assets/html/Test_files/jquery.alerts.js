...
(function($) {
	...
	$.alerts = {
		...
		overlayColor: '#878787',               // base color of overlay
		...
		_reposition: function() {
			...
			if( $.browser.msie){
				var temp=$(".tbl_outerline").offset();
				var temp1=$(document).height()/3+'px';
					if(temp==null){
						var temp1=$(document).height()/3+'px';
					}else{
						var temp1=$(document).height()/2+'px';
						if(temp.top<1000)
						{
							temp1=$(document).height()/3+'px';
						}
						else{
						temp1=temp.top;
						}
